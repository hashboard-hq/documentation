---
description: DuckDB allows you to use Hashboard without setting up any external database.
---

# DuckDB

[DuckDB](https://duckdb.org) allows you to use Hashboard without
setting up any external database.

DuckDB is a unique database amongst the types Hashboard supports. Traditional
...
likely already have: CSVs, JSON arrays, and more. DuckDB also runs on Hashboard's
...
to get their data into Hashboard without setting up any servers.** DuckDB is _not_
a good choice if your dataset is large, or needs to be updated in realtime
from an external system.

### How to get set up

Hashboard pre-configures a DuckDB connection for every project. You are already
ready to start exploring data files in your Hashboard project.
