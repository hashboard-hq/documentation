---
description: Athena is an Amazon Web Services SQL query engine that allows you to do analytics on files stored in S3
---
...
formats, it can be slow. Hashboard assumes you have data in a fast data warehouse
...
metadata can be very slow, so development in Hashboard can be very slow 2. the
...
Hashboard's caching. Amazon has a few good articles on
...
<Callout>
  ...
  querying metadata and getting lists of tables faster in Hashboard during
  development.
</Callout>
...
- **AWS Secret Access Key:** The IAM user's scret access key, generated in step
  4 [above](/docs/database-connections/athena#create-an-aws-iam-user-for-hashboard).
