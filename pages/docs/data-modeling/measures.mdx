...
### Deriving measures from other measures

Custom measures can refer to other measures to avoid duplicating logic.
For instance, a **Profit** measure could be defined by deducting **Costs** from **Revenue**.

```sql filename="SQL for Profit measure"
{{ revenue }} - {{ cost }}
```

SQL can reference other measures by surrounding the measure's `alias` in curly braces.
