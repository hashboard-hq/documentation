...
To prepare datetime data for consumption in Hashboard, we recommend storing it in a timezone unaware datatype for a specific localization. For example create a column named `datetime_UTC` with a [`DATETIME`](https://docs.snowflake.com/en/sql-reference/data-types-datetime#datetime) or [`TIMESTAMP_NTZ`](https://docs.snowflake.com/en/sql-reference/data-types-datetime#timestamp-ltz-timestamp-ntz-timestamp-tz) type in Snowflake.
