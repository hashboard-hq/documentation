...
Deployment of all code-controlled resources is still available by supplying a `--full_rebuild` flag:
...
The v3 CLI introduces a `hb init` command, which creates a `.hbproject` file in your filesystem. This file is to be placed at the root of your Hashboard project and ensures that your resource identities are stable, regardless of where you are running your `hb build` command.
...
3. In your scripts / CI actions, migrate commands as follows:
    ...
    | `hb preview/deploy --dbt` | Set `--dbt-root` when running `hb init`, and then use `hb build`<br/><br/>To exclude dbt models, use `hb build --no-dbt` |
