...
Below are all the options that can be defined within the .hbproject file
...
Any value under dbtOptions can also be set using the corresponding environment variable

| .hbproject key        | env variable                | usage                                                                                                                             |
| --------------------- | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| useDefer              | HB_DBT_USE_DEFER            | Boolean value indicating if Hashboard should look for tables deferred to production in dbt.                                       |
| includeDbNameInSchema | HB_DBT_INCLUDE_DB_IN_SCHEMA | Boolean value indicating if schemas should be qualified with a database name, primarily used when doing cross database deferrals. |
| accountId             | HB_DBT_CLOUD_ACCOUNT_ID     | The dbt Cloud account id to use when fetching a specific run if creating a build with the --dbt-cloud-run-id flag                 |
