...
```
You can explore data sources further using the [`tables`](./tables) command and upload files directly to Hashboard using the `datasource upload` command described below. To configure new connections check out the [Hashboard guide](/docs/database-connections) for connecting to your database.
