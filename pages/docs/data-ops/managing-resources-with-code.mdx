...
An easy way to get started building a configuration file is to export it from an existing Hashboard resource.

- Data Models:
  1. Navigate to the [Data Models](https://hashboard.com/app/p/data-models) page
...
To protect specific resources to only be editable with DataOps, add the `preventUpdatesFromUI` flag set as `true` to any config. This prevents users from unintentionally updating critical resources from the Hashboard web application and gives your team greater control over valuable long lived resources that are core to your business needs.
