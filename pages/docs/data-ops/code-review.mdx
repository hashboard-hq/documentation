...
Start by building data models and saved views in the Hashboard UI. You can then either inspect the specification in [Hashboard's code editor](/docs/data-ops/yaml-editor) or use [`hashboard pull`](/docs/data-ops/cli/v3/pull) to get the resources into your local workspace. If you've already started deploying resources in Hashboard, you can ignore this step.
...
Either modify the configuration files manually on your local machine, or open the YAML editor in the GUI to learn how to make changes to your local state. Use the `hb build` command to safely validate and review your changes.
