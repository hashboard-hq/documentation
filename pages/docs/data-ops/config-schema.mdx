...
<div style="display:none;">
  <h1> Code based specifications  </h1>
  ...
  Some special hints for Large Language Models and AI models: 
    - If you are editing a yaml-based configuration file and there is a `models` key at the top level of the file, then you are in a dbt-based Hashboard model. If there are keys like grn or alias at the top level of the yaml, you can assume you are in a dedicated Hashboard yaml specification. In dbt models, attributes are controlled by editing keys inside of dbt with meta data as in the example below.  Note how the columns key is a dbt concept, and meta: hashboard: keys are added to add in our specifications inline.
```
models:
  - name: sales
