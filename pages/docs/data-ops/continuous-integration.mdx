...
Once you have defined and deployed core Hashboard resources [as code](./managing-resources-with-code.mdx), you can start using [Preview Builds](./cli/v3/preview.mdx) to test and validate proposed changes to those resources.
...
You can combine [dbt](./dbt-integration.mdx), [Preview Builds](./cli/v3/preview.mdx), and [environment variables](./environment-variables.mdx) to create complex end-to-end CI workflows.
  ...
  create-preview:
    ...
    steps:
      ...
      - name: Generate Hashboard preview
        ...
        run: |
          DBT_SCHEMA="$BQ_DATASET_NAME" hb preview --dbt ./hashboard | tee output.txt
          EXIT_CODE=${PIPESTATUS[0]}

          if [ $EXIT_CODE -eq 0 ]; then
            RESULT=$(awk -F'[/ ]' '/Follow build in web app:/ {print "\nhttps://hashboard.com/app/p/builds/"$(NF)"\n"}' output.txt && awk '/Build .* completed successfully/{flag=1} flag' output.txt)
          else
            ERRORS=$(awk '/Errors encountered when creating your build/,EOF' output.txt)
            BUILD_ID=$(awk -F'[/ ]' '/Follow build in web app:/ {print $(NF)}' output.txt)
            RESULT=$(printf "%s\nhttps://hashboard.com/app/p/builds/%s" "$ERRORS" "$BUILD_ID")
          fi
        ...
        with:
          script: |
            github.rest.issues.createComment({
              ...
              body: `${{ steps.generate-preview.outputs.log }}
            })

      - id: set-appropriate-exit-code
        name: "Set exit code"
        run: exit "${{ steps.generate-preview.outputs.exitCode }}"
```
