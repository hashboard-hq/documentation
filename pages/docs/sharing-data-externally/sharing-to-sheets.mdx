...
Before you can export data to Google Sheets, you'll need to grant Hashboard access to update your spreadsheets.
...
4. You will be redirected back to your Hashboard profile and see a success message.
...
### Limitations of Google Sheets reports

Spreadsheets in Google Sheets are limited to 10 million cells across all tabs in a single spreadsheet.
Hashboard will automatically truncate rows of data if it exceeds this limit. 

In addition, we limit the number of rows exported from underlying data to a maximum of 100,000. Generally, we 
recommend exporting aggregated chart data instead of underlying data, as performance can be slow when exporting
large amounts of underlying data.
...
- You'll receive an email notification with the error details.
